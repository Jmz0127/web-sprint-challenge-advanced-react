...
Functional components are stateless in the sense that they do not play with the component's state. For example, a functional component that only renders props passed down to it is an example of a functional components. Any component that is independent of state is a functional or stateless component.
Stateful will modify state of a component, for example class components are stateful. When I say stateful I mean there is something in that component that modifies state - for example, something with an eventHandler.

2. When does a componentWillMount function be called? What about a componentWillUpdate?

componentWillMount will be called during the mounting phase of the lifecycle. Its typically sandwiched between the constructor and the render functions, used to fetch some sort of data before rendering anything to the end user.
componentWillUpdate will be called during the updating phase of the lifecycle, right before it receives new props/state values.

3. Define stateful logic.

Essentially its the use, modification, and general logic of state within a component. Really any code that uses and involves state could fall under stateful logic, whether that be a text input field that updates state as a user types into it, or some sort of onClick event.

4. What are the three step of creating a successful test? What is done in each phase?

Arrange - What component are we rendering (set the context), essentially what items are we testing? For Arrange, we are rendering a component.
Act - What is it that the user would do to the item that is arranged? In other words, what specific things are we testing that we will implement a user action on? For Act, we are highlighting an element within the component and then replicating a user action on it.
Assert - Once we have acted on the arranged item, what is the desired result? For Assert, we are expecting to see or not to see something, whether its an error, a message, or other desired outcome of a test.
